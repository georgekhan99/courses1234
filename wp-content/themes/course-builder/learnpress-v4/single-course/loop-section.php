				...
				<li class="<?php echo esc_attr( implode( ' ', $item->get_class() ) ); ?>" data-id="<?php echo esc_attr( $item->get_id() ); ?>">
                        ...
                        <div class="meta-rank">
                            <?php if ( $item->get_item_type() == 'lp_quiz' ) { ?>
                                <div class="rank"><i class="fa fa-puzzle-piece"></i><?php echo '<span class="label">' . esc_html__( 'Quiz', 'course-builder' ) . '</span>' . $section->get_position() . '.' . $count[ $post_type ]; ?></div>
                            <?php } elseif ( $item->get_item_type() == 'lp_assignment' ) {?>
                                <div class="rank"><?php echo '<span class="label">' . esc_html__( 'Assign', 'course-builder' ) . '</span>' . $section->get_position() . '.' . $count[ $post_type ]; ?></div>
                            <?php } else { ?>
                                <div class="rank"><i class="fa fa-file-archive"></i><?php echo '<span class="label">' . esc_html__( 'Lecture', 'course-builder' ) . '</span>' . $section->get_position() . '.' . $count[ $post_type ]; ?></div>
                            <?php } ?>
                        </div>
